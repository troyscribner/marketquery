...
class MarketDataClient:
    ...
    def __init__(
        ...
        Args:
            provider: Name of the provider to use (default: checks MARKETQUERY_DEFAULT_PROVIDER env var, then "yahoo")
            api_key: Optional API key for the provider
            premium_api_key: Optional premium API key for the provider
            cache_dir: Optional directory to store cached data
        """
        # If no provider specified, check environment variable or default to yahoo
        if provider is None:
            provider = os.getenv("MARKETQUERY_DEFAULT_PROVIDER", "yahoo")
            
        self.provider = self._get_provider(provider, api_key, premium_api_key)
        self.cache = CacheManager(cache_dir)

    def _get_provider(
        self, provider: str, api_key: Optional[str], premium_api_key: Optional[str]
    ) -> BaseProvider:
        """
        ...
        if api_key is None:
            if provider == "tiingo":
                ...
                if (
                    premium_api_key is None
                ):  # Only get from env if not provided in constructor
        ...
        **kwargs,
        ...
        if uncached_symbols:
            if kwargs.get("progress", True):
                print(
                    f"Downloading {len(uncached_symbols)} symbols: {', '.join(uncached_symbols)}"
                )
        ...
        if kwargs.get("progress", True):
            if cached_symbols:
                print(
                    f"Loaded {len(cached_symbols)} symbols from cache: {', '.join(cached_symbols)}"
                )
            if uncached_symbols:
                print(
                    f"Downloaded {len(uncached_symbols)} symbols: {', '.join(uncached_symbols)}"
                )
